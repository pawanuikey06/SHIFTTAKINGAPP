...
const sendResetPasswordEmail = require("../EmailService/attachements/ResetPassword");
const sendUpdatePasswordEmail = require("../EmailService/attachements/UpdatePassword");
...
router.post("/reset", async (req, res) => {
  try {
    ...
    res.status(200).json("Check your email for reset password");
  } catch (error) {
    res.status(500).json(error);
  }
});
...
router.post("/update-password", async (req, res) => {
  try {
    ...
    await User.finByIdAndUpdate(user._id, {
